...
const cria = async (model) => {
	...
	if (!categoriaDB) {
		return {
			sucesso: false,
			mensagem: "operação não pode ser realizada",
			detalhes: ["Não existe categoria cadastrada para o categoria id informado"],
		};
	}
...
const pesquisaPorFiltros = async (filtros) => {
	...
	const resultadoDB = await produto.find(filtroMongo).populate("produto").populate("fornecedor").populate("categoria");
...
const deleta = async ({ fornecedorId, produtoId, usuarioId }) => {
	const [fornecedorDB, produtoDB] = await Promise.all([fornecedor.findById(fornecedorId), produto.findById(produtoId)]);
	...
	await Promise.all([categoriaDB.save(), fornecedorDB.save(), produto.deleteOne({ _id: produtoId })]);
	...
	await Promise.all(
		categoriaArray.map(async (item) => {
			...
			await categoria.updateOne({ _id: item._id }, { produtos: categoriaProduto });
		})
	);
